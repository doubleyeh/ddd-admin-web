    ...
    <n-card :bordered="false" class="h-full">
        <div class="flex items-center mb-4 flex-nowrap">
            ...
            <n-space class="ml-4">
                ...
                <n-button v-if="userStore.hasPermission('tenant:create')" type="success" class="ml-auto"
                    @click="handleCreate">
                    新增租户
                </n-button>
            </n-space>
        ...
        <n-data-table :columns="columns" :data="tableData" :loading="loading" :pagination="pagination" remote
            ...
            :row-key="(row: TenantDTO) => row.id" class="flex-1" />

        <n-modal v-model:show="showModal" :mask-closable="false" preset="dialog" :title="isEdit ? '编辑租户' : '新增租户'"
