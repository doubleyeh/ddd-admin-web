    ...
    <n-card :bordered="false" class="h-full">
        <div class="flex items-center mb-4 flex-nowrap">
            <n-space wrap-item>
                ...
                <n-select v-model:value="query.state" :options="stateOptions" placeholder="用户状态" clearable
                    class="w-28" />
            </n-space>
            ...
            <n-space class="ml-4">
                ...
                <n-button v-if="userStore.hasPermission('user:create')" type="success" class="ml-auto"
                    @click="handleCreate">
                    新增用户
                </n-button>
            </n-space>
        ...
        <n-data-table :columns="columns" :data="tableData" :loading="loading" :pagination="pagination" remote
            @update:page="handlePageChange" @update:pageSize="handlePageSizeChange" :row-key="(row: UserDTO) => row.id"
            class="flex-1" />

        <n-modal v-model:show="showModal" :mask-closable="false" preset="dialog" :title="isEdit ? '编辑用户' : '新增用户'"
            :style="{ width: '500px' }" positive-text="确认" negative-text="取消" @positive-click="handleSave">
            <n-form ref="formRef" :model="formModel" :rules="formRules" label-placement="top" class="mt-4">
...
async function fetchTableData() {
    loading.value = true
    try {
        const res = await UserApi.findPage(
            pagination.page - 1,
            pagination.pageSize,
            {
                ...query
